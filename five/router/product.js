...
r.post("/v1/join_cart",(req,res)=>{
	pool.query("select count from mncake_shopping_cart where user_id=? and laptop_id=?",[req.body.user_id,req.body.laptop_id],(err,result)=>{
		if(err) throw err;
		if(result.length)
		{   var a=result[0].count;
		    pool.query("update mncake_shopping_cart set count=? where user_id=? and laptop_id=?",[req.body.count/1+a,req.body.user_id,req.body.laptop_id],(err1,result1)=>{
		    	if(err1) throw err1;
